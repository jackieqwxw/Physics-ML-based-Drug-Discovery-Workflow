...
def convert_sdf_to_pdb(sdf_path, pdb_path):
    ...
    molecule.write("pdb", pdb_path, overwrite=True)
...
shape = "84,124,1" 
shells = "62" 
scaler = 'train_scaler.scaler'

model = '62shell_saved-model.h5'
shape = shape = [int(x) for x in shape.split(",")]
...
for index, row in data.iterrows():
    ...
    # rec_fpath = '1a30_protein.pdb'
    # lig_fpath = '1a30_ligand.sdf'
    pdb_file =lig_fpath[:-4]+ '.pdb'
    
    convert_sdf_to_pdb(lig_fpath, pdb_file)

    #print(rec_fpath,'    ',lig_fpath,'   ', pdb_file)

    X_feat = generate_features(rec_fpath, pdb_file)

    X_test_std = scaler.transform(X_feat.reshape(1, -1)).reshape([-1] + shape)
    pred_pKa = model.predict(X_test_std).ravel()
    pred.append(pred_pKa[0])
    print(i)
    i = i +1
...
# rec_fpath = '1bcu_protein.pdb'
# lig_fpath = '1bcu_ligand.sdf'
