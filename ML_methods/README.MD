...
```
---
Each method folder typically contains:

Documentation describing the method

Installation instructions

Scripts used to run predictions
...
The workflows provided here aim to reproduce the machine learning benchmarks reported in the manuscript. Whenever possible, the original implementations provided by the authors of each method were used without modification.

Due to GitHub file size limitations, large model files and datasets are not stored directly in this repository. Instead, instructions are provided for downloading these resources from their original sources.

---
## Notes

This repository focuses on documenting the workflows used to evaluate the machine learning methods in the context of large-scale virtual screening benchmarks.

For detailed implementation of each method, users should refer to the original repositories maintained by the respective authors.
